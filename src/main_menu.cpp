                            ...
                            int iMenuOffsetY )
{
    ...
    mvwprintz( w_open, window_height - 1, 0, c_yellow, string_format( _( "Tip of the day: " ) ) );
    mvwprintz( w_open, window_height - 1, 15, c_light_cyan, string_format( _( " %s" ), vdaytip ) );

    // iLine++;
    mvwprintz(w_open, 4, window_width - 24, c_dark_gray, string_format( _( "Version: %s" ), getVersionString() ) );
    //center_print( w_open, 0, c_red, string_format( _( "Version: %s" ), getVersionString() ) );
...
bool main_menu::new_character_tab()
{
    ...
    bool start = false;
    while( !start && sel1 == 0 && ( layer == 2 || layer == 3 ) ) {
        print_menu( w_open, 0, iMenuOffsetX, iMenuOffsetY );
        if( layer == 2 && sel1 == 0 ) {
