...
void main_menu::print_menu( const catacurses::window &w_open, int iSel, const point &offset )
{
    ...
    } else { center_print( w_open, iLine++, cColor1, mmenu_title[0] ); }
    ...
    //fprintf(stderr, "vMenuItems.size()=[%d]\n", vMenuItems.size());
...
{
    // ASCII Art
    mmenu_title = load_file( PATH_INFO::title( ), _( "Cataclysm" ) );
    // mmenu_title = load_file( "title/", "en.title" );
    // MOTD
...
bool main_menu::load_character_tab( bool transfer )
{
    ...
    while( !start && sel1 == 1 && ( layer == 2 || layer == 3 ) ) {
        print_menu( w_open, transfer ? 2 : 1, menu_offset );
        if( layer == 2 && sel1 == 1 ) {
...
{
    while( sel1 == 2 && ( layer == 2 || layer == 3 || layer == 4 ) ) {
        print_menu( w_open, sel1, menu_offset );
